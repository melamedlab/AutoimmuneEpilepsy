...
### Biomart was retired by ENSEMBL :( 
##### We have saved a copy of the final mart, but it can not be made public by default on github. If you would like the file please contact the authors of the paper.
...
# write.csv(mart_retrieved, "saved_mart.csv")
mart_retrieved <- read.csv("saved_mart.csv", row.names = 1)
...
```{r}
 ...
 nt_proteins <- data %>%
   #filter(gene_symbol %in% c(channel_genes$`Approved symbol`)) %>%
   filter(gene_symbol %in% c(mart_retrieved$hgnc_symbol, "STXBP5",
                              "ERC2", "NRXN3", "NF1")) %>%
   # STXBP5, NRXN3 and ERC2 were genes inside the GO code previously, but were not
   ## retrieved in the Biomart legacy code and we are adding them manually here
   filter(gene_symbol != "EFR3A") %>%
   # While now classified as neurotransmitter associated, at time of manuscript drafting
   ## this was not classified with those GO codes
   rowwise() %>%
   mutate(total = sum(c_across(Melamed_PhIP8_Plate8_06_A6_MS1:Melamed_PhIP8_Plate8_49_E1_Ctl6))) %>%
   filter(total > 0)
...
```{r}
 ...
 write.csv(rbind(log10(as.matrix(heatdf_channel[order(row.names(heatdf_channel)), ]) + 1), log10(as.matrix(heat_nt[order(row.names(heat_nt)), ]) + 1),
                 log10(as.matrix(heat_memb[order(row.names(heat_memb)), ]) + 1)
 ), "../output/source_data_Figure2A.csv")
```
...
write.csv(ggb$data, "../output/source_data_Figure2B.csv")
```
...
write.csv(ggc$data, "../output/source_data_Figure2C.csv")
```
