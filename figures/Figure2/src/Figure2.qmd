...
stats_df <- stats_df %>% filter(p.adj <= 0.05) %>% mutate(y.position = c(0.2, 0.22,0.24)) %>%
  ...
  mutate(group1 = ifelse(group1 == "Epilepsy_normal", "AED-Responsive\nEpilepsy",
                            ifelse(group1 == "Epilepsy_SN", "snAE", group1)))%>%
  mutate(group2 = ifelse(group2 == "Epilepsy_normal", "AED-Responsive\nEpilepsy",
                         ifelse(group2 == "Epilepsy_SN", "snAE", group2)))

## Panel B
ggb <- data_ready %>%
  mutate(Condition = ifelse(Condition == "Epilepsy_normal", "AED-Responsive\nEpilepsy",
                            ifelse(Condition == "Epilepsy_SN", "snAE", Condition))) %>%
  mutate(Condition = factor(Condition, c("Control", "snAE", "AED-Responsive\nEpilepsy",
                                         "MS"))) %>%
