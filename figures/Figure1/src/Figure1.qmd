...
cor(data, method = "pearson") %>% as.data.frame() %>%
  rownames_to_column("rows") %>%
  pivot_longer(cols = -rows) %>%
  filter(rows != name) %>%
  dplyr::summarize(mean = mean(value))
library(corrplot)
tmp <-cor(data, method = "pearson")
rownames(tmp) <- gsub("\\.", "-", gsub("\\.\\..*", "", rownames(tmp)))
colnames(tmp) <- gsub("\\.", "-", gsub("\\.\\..*", "", colnames(tmp)))

pdf("../output/Correlation_cytokine_plot.pdf", width = 10, height = 10)
corrplot::corrplot(tmp, type = 'upper', order = "hclust", col = rev(COL2("RdBu", n = 200)), tl.col = "black", tl.cex = 0.8)
dev.off()
...
pcv <- ((cyto.pca$sdev^2) / sum(cyto.pca$sdev^2)) * 100

metadata_tmp <- metadata %>%
  mutate(Group = gsub("snAAE Post Steroids", "snAE\nPost Steroids", Group)) %>%
      mutate(Group = gsub("Epilepsy", "ASM\nResponsive\nEpilepsy", Group),
             Group = gsub("snAAE", "snAE", Group))
gga <- ggbiplot(cyto.pca, choices = c(1,2), var.axes=F, groups=metadata_tmp$Group,
  ...
  scale_color_manual(values=c("snAE"="#BF5448", "CTL"="#9048BF",
                                  ...
                                  "snAE\nPost Steroids" = "#DCD921"), name = "Group") +
  ylab(paste0("PC2 (", round(pcv[2], 1), "%)")) +
  xlab(paste0("PC1 (", round(pcv[1], 1), "%)"))
gga
...
cbind.data.frame(metadata, cyto.pca$x %>% as.data.frame() %>%
                   dplyr::select(PC1), data) %>%
  pivot_longer(cols = colnames(data)) %>%
  group_by(name) %>%
  dplyr::summarize(cor = cor(PC1, value), R2 = cor(PC1, value)^2) %>%
  ungroup() %>%
  dplyr::summarize(mean_cor = mean(abs(cor)), mean_R2 = mean(R2), high_R2 = sum(ifelse(R2>0.5, 1, 0)), super_high_R2 = sum(ifelse(R2>0.7, 1, 0)))
...
write.csv(gga$data %>% select(1:3), "../output/source_data_Figure1A.csv")
write.csv(ggb$data, "../output/source_data_Figure1B.csv")

plots_c_on <-c(ggout[order(match(names(ggout),cytokine_order))],
               ggout2[order(match(names(ggout2),cytokine_order))])
lets <- LETTERS[-1:-2]
data.out <- data.frame()
for(i in 1:length(plots_c_on)){
  active_let <- lets[i]
  data.out <- rbind.data.frame(data.out, plots_c_on[[i]]$data %>% mutate(panel = active_let))
}
write.csv(data.out,
          paste0("../output/source_data_Figure1C_V.csv"))
```
...
ggarrange(plotlist = ggout_supp[order(match(names(ggout_supp),cytokine_order))], ncol =6, nrow = 8,labels = c(LETTERS, sapply(LETTERS, function(x) paste0(x, LETTERS)))[1:length(ggout_supp)])
ggsave(paste0(out_dir, "SupplementalFigure1.pdf"), width = 18, height = 24)



plots_c_on <- ggout_supp[order(match(names(ggout_supp),cytokine_order))]
lets <- c(LETTERS, sapply(LETTERS, function(x) paste0(x, LETTERS)))[1:length(ggout_supp)]
data.out <- data.frame()
for(i in 1:length(plots_c_on)){
  active_let <- lets[i]
  data.out <- rbind.data.frame(data.out, plots_c_on[[i]]$data %>% mutate(panel = active_let))
...
write.csv(data.out,
          paste0("../output/source_data_Supplemental_Figure1.csv"))
```
