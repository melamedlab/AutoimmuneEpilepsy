...
metadata_tmp <- metadata %>%
  mutate(Group = gsub("snAAE Post Steroids", "snAAE\nPost Steroids", Group)) %>%
...
gga <- ggbiplot(cyto.pca, choices = c(1,2), var.axes=F, groups=metadata_tmp$Group,
  ...
  theme_classic() +
  scale_color_manual(values=c("snAAE"="#BF5448", "CTL"="#9048BF",
...
for(i in unique(long_cytokine_full$name)){
  ...
  plot_data <- long_cytokine_full %>%
      ...
      mutate(Group = factor(Group, c("snAAE",  "snAAE\nPost\nSteroids",
                                     "CTL", "ASM\nResponsive\nEpilepsy",
                                     "MS"))) 
  ...
  if(nrow(tmp_stats %>%
    ...
    filter(p.adj.signif != "ns")
       if("snAAE" %in% tmp_stats_sig$group1 & "CTL" %in% tmp_stats_sig$group2){
